#include "cpu/pred/pathperceptron.hh"
...
{  
  ...
  weights.assign(numberOfPerceptrons,std::vector<unsigned>(globalPredictorSize + 1, 0));
...
bool PathPerceptron::lookup(ThreadID tid, Addr branch_addr, void * &bp_history)
{
  ...
  for (int j = 1; j <= globalPredictorSize; j++) {
	...
	if (result) selectiveRegister_new[k_j + 1] += weights[curPerceptron][j];
	else            selectiveRegister_new[k_j + 1] -= weights[curPerceptron][j];
  }
  ...
  selectiveGlobalHistory[tid] = (selectiveGlobalHistory[tid] & historyRegisterMask);
  return result;
}
